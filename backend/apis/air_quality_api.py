...
# obtains air quality info for 1 zip code via api call. The api call returns info for multiple days, so we will return only the air quality rating number and qualitative rating  for each day, as a list.
# example return format: [ {Number: 1, Name: good}, {Number: 2, Name: medium}, {Number: 1, Name: good}]    (where each object represents a day)
def get_aqi_category(zip_code, date, distance=25):
    res = []
    url = f"https://www.airnowapi.org/aq/forecast/zipCode/?format=application/json&zipCode={zip_code}&date={date}&distance={distance}&API_KEY=79E1FA09-525F-4ABB-88D0-8FF01059A3C8"
    ...
    for zip_map in name_and_category_map:
        ...
        #print(zip_map)
        print(f"zip map: {zip_map}")
        res.append(zip_map['Category'])

    return res


# call get_aqi_category on every zip in unique zips, create a hashmap mapping zipcode to its corresponding rating (first day), and return this hashmap
def get_aqi_category_all_zips():
    ret = {}
    for zip in unique_zips:
        res = get_aqi_category(zip, "2024-09-06")
        print(res)
        if len(res) > 0:
            ret[zip] = res[0]["Number"]
    return ret
